...
function SendTechSkill(){
	...
	var url = "/mongo_add_technical_skill?name=" + $("#techSkillName").val() + "&category=" + $("#techSkillCategory").val() + "&level=" + $("#techSkillLevel").val() + "&importance=" + $("#techSkillImportance").val();
  console.log(url);
  ...
  function DeleteTechSkill(x){
	var scope = $(x).parent().parents()[0];
  	var xhttp = new XMLHttpRequest();
  	var url = "/mongo_delete_technical_skill?name=" + $(scope).find("#submitedTechSkillName").val() + "&category=" + $(scope).find("#submitedTechSkillCategory").val() + "&level=" + $(scope).find("#submitedTechSkillLevel").val() + "&importance=" + $(scope).find("#submitedTechSkillImportance").val();
    console.log(url);
  	xhttp.open('GET', url, true);
  	xhttp.send();
  	}
